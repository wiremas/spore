...
K_TOOL_CMD_NAME="sporeScaleToolCmd"
K_CONTEXT_NAME="sporeScaleContext"
...
class ScaleToolCmd(spore_tool.SporeToolCmd):
    """ spore base tool command - abstract tool command class ment to """

    def __init__(self):
        super(ScaleToolCmd, self).__init__()
    ...
    @staticmethod
    def creator():
        return ompx.asMPxPtr(ScaleToolCmd())
...
class ScaleContext(spore_tool.SporeContext):

    def __init__(self):
        super(ScaleContext, self).__init__()
        self._setTitleString('ScaleContext')
    ...
    def toolOnSetup(self, event):
        super(ScaleContext, self).toolOnSetup(event)
        print 'derivedSetup'
        print self.__dict__
        self.canvas = canvas.CircularBrush(self.state)

    def toolOffCleanup(self, event):
        super(ScaleContext, self).toolOffCleanup(event)



class ScaleContextCommand(ompx.MPxContextCommand):
    def __init__(self):
        ompx.MPxContextCommand.__init__(self)

    def makeObj(self):
        return ompx.asMPxPtr(ScaleContext())

    @staticmethod
    def creator():
        return ompx.asMPxPtr(ScaleContextCommand())
